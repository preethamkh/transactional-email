...
| 4 | Decision: approve **one Mandrill block (~US$20/25k emails, month-to-month)** for a real send test — or accept "documented, not tested" for Mailchimp sending | Manager (budget) | Without it, the Mailchimp sending/reporting comparison is impossible, and the BA's requirement can only be half-met. Either answer is a useful finding; get the decision explicitly. | Day 0–1 |
...
> 3. **Budget (optional)**: approval for one Mailchimp transactional block (~US$20/month-to-month) ONLY if you want a real Mailchimp-side send/reporting demonstration. Default plan is document-only — retrieval and comparison proceed without it.
